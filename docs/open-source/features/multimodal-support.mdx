...
const messages = [
  { role: "user", content: "Help me remember which dishes I liked." },
  {
    role: "user",
    content: {
      type: "image_url",
  ...
  { role: "user", content: "I'm allergic to peanuts and prefer vegetarian meals." }
];

const result = await client.add(messages, { userId: "user123" });
console.log(result);
```
...
```typescript
const messages = [
  { role: "user", content: "Store this receipt information for expenses." },
  {
    role: "user",
    content: {
      type: "image_url",

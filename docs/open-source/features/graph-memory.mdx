...
<Info icon="clock">
**Time to implement:** ~10 minutes · **Prerequisites:** Node.js 18+, Neo4j Aura DB (free tier)
</Info>

Provision a free [Neo4j Aura](https://neo4j.com/product/auradb/) instance, copy the Bolt URI, username, and password, then follow the steps below.
...
```typescript
// Pass threshold in the graphStore config
const config = {
  enableGraph: true,
  graphStore: {
    provider: "neo4j",
    config: { url: "...", username: "...", password: "...", threshold: 0.75 },
  },
};
```
  </Accordion>
...
```typescript
await memory.add(messages, { userId: "demo-user", enableGraph: false });
const results = await memory.search("marketing partners", { userId: "demo-user", enableGraph: false });
```
  </Accordion>

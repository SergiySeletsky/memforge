...
<Info>
  Begin with the <Link href="/open-source/node-quickstart">Node.js quickstart</Link> to install the SDK, configure dependencies, and validate memory reads/writes locally.
</Info>
...
<CardGroup cols={1}>
  <Card title="Deploy with Docker Compose" icon="server" href="/cookbooks/companions/local-companion-ollama">
    Reference deployment with Qdrant and the TypeScript SDK.
  </Card>
</CardGroup>
...
<Note>
  ...
  Override any component via the <Link href="/open-source/configuration">configuration guide</Link>.
</Note>
...
<CardGroup cols={2}>
  ...
  />
  <Card
    title="Run the Node.js Quickstart"
    description="Install the TypeScript SDK and persist your first local memory."
    icon="terminal"
    href="/open-source/node-quickstart"
  />
</CardGroup>

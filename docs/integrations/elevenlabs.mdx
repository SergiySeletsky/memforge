...
```bash
npm install mem0ai @elevenlabs/elevenlabs-js
```
...
- `MemoryClient` from Mem0 for memory operations
- ElevenLabs client for voice interaction
...
async function main() {
  ...
  if (!AGENT_ID) {
    process.stderr.write("AGENT_ID environment variable must be set\n");
    process.exit(1);
  }

  if (!USER_ID) {
    process.stderr.write("USER_ID environment variable must be set\n");
    process.exit(1);
  }

  if (!API_KEY) {
    process.stderr.write("ELEVENLABS_API_KEY not set, assuming the agent is public\n");
  }

  if (!MEM0_API_KEY) {
    process.stderr.write("MEM0_API_KEY environment variable must be set\n");
    process.exit(1);
  }
}
```
...
```typescript
  ...
  const elevenLabsClient = new ElevenLabsClient({ apiKey: API_KEY });
...
- Create a `MemoryClient` instance for Mem0 interactions
...
```typescript
  ...
  async function addMemories(parameters: { message: string }) {
    const message = parameters.message;
    await mem0Client.add(message, { userId: USER_ID! });
    return "Memory added successfully";
  }

  async function retrieveMemories(parameters: { message: string }) {
    const message = parameters.message;

    // For Platform API, userId goes in filters
    const filters = { AND: [{ userId: USER_ID! }] };

    // Search for relevant memories using the message as a query
    const results = await mem0Client.search(message, { filters });

    // Extract and join the memory texts
    const memories = (results['results'] || []).map((r: any) => r['memory']).join(' ');
...
```typescript
  ...
  clientTools.register("addMemories", addMemories);
  clientTools.register("retrieveMemories", retrieveMemories);
```
...
```typescript
  ...
  const conversation = new Conversation(elevenLabsClient, AGENT_ID!, {
    // Assume auth is required when API_KEY is set
    requiresAuth: Boolean(API_KEY),
    clientTools,
    onAgentResponse: (response: string) => console.log(`Agent: ${response}`),
    onAgentResponseCorrection: (original: string, corrected: string) => console.log(`Agent: ${original} -> ${corrected}`),
    onUserTranscript: (transcript: string) => console.log(`User: ${transcript}`),
    // onLatencyMeasurement: (latency: number) => console.log(`Latency: ${latency}ms`),
  });
```
...
```typescript
  ...
  console.log(`Starting conversation with userId: ${USER_ID}`);
  await conversation.startSession();

  // Handle Ctrl+C to gracefully end the session
  process.on('SIGINT', () => conversation.endSession());

  // Wait for the conversation to end and get the conversation ID
  const conversationId = await conversation.waitForSessionEnd();
...
2. **Agent retrieves memories**:
   ```typescript
   // Agent calls retrieveMemories
   const memories = await retrieveMemories({ message: "user's favorite color" });
...
6. **Agent stores new information**:
   ```typescript
   // Agent calls addMemories
   await addMemories({ message: "The user's favorite color is green" });
   ```

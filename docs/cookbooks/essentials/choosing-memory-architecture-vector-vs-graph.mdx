...
// Add employee info
await client.add([{ role: 'user', content: 'Emma is a software engineer in Seattle' }], { userId: 'company_kb' });
await client.add([{ role: 'user', content: 'David is a product manager in Austin' }], { userId: 'company_kb' });
```

Now let's search for Emma's role:

```typescript
const results = await client.search("What does Emma do?", { filters: { user_id: 'company_kb' } });
...
```typescript
await client.add([{ role: 'user', content: 'Emma works with David on the mobile app redesign' }], { userId: 'company_kb' });
await client.add([{ role: 'user', content: 'David reports to Rachel, who manages the design team' }], { userId: 'company_kb' });
```
...
```typescript
await client.add(
  [{ role: 'user', content: 'Emma works with David on the mobile app redesign' }],
  { userId: 'company_kb', enableGraph: true }
);

await client.add(
  [{ role: 'user', content: 'David reports to Rachel, who manages the design team' }],
  { userId: 'company_kb', enableGraph: true }
);
```

When you set `enableGraph: true`, Mem0 extracts entities and relationships:
...
const results = await client.search(
  "Who is Emma's teammate's manager?",
  { filters: { user_id: 'company_kb' }, enableGraph: true }
);

console.log(results.results[0].memory);
console.log('\nRelationships found:');
for (const rel of results.relations ?? []) {
  console.log(`  ${rel.source}, ${rel.target} (${rel.relationship})`);
}
```
...
await client.add([{ role: 'user', content: 'Emma specializes in React and TypeScript' }], { userId: 'company_kb' });
await client.add([{ role: 'user', content: 'David has 5 years of product management experience' }], { userId: 'company_kb' });

// Relationships - use graph memory
await client.add(
  [{ role: 'user', content: 'Emma and David work together on the mobile app' }],
  { userId: 'company_kb', enableGraph: true }
);

await client.add(
  [{ role: 'user', content: 'David reports to Rachel' }],
  { userId: 'company_kb', enableGraph: true }
);

await client.add(
  [{ role: 'user', content: 'Rachel runs weekly team syncs every Tuesday' }],
  { userId: 'company_kb', enableGraph: true }
);
```
...
const facts = await client.search("What are Emma's skills?", { filters: { user_id: 'company_kb' } });
console.log(facts.results[0].memory);
```
...
console.log(graphResults.results[0].memory);
```
...
Graph memory adds processing time and cost. When you call `client.add()` with `enableGraph: true`, Mem0 makes extra LLM calls to extract entities and relationships.
...
await client.add(
  [{ role: 'user', content: 'Emma mentors two junior engineers on the frontend team' }],
  { userId: 'company_kb', enableGraph: true }
);

// Temporary notes - skip graph, not worth the cost
await client.add(
  [{ role: 'user', content: 'Emma is out sick today' }],
  { userId: 'company_kb', runId: 'daily_notes' }
);
```
...
```typescript
// Per-call (recommended to start):
await client.add([{ role: 'user', content: 'Emma works with David' }], { userId: 'company_kb', enableGraph: true });
await client.search('team structure', { filters: { user_id: 'company_kb' }, enableGraph: true });
...
await client.add([{ role: 'user', content: 'Emma mentors Jordan' }], { userId: 'company_kb' });
```

...
const config = {
  vectorStore: {
    ...
    config: {
      ...
      host: 'localhost',
      port: 6333,
      embeddingModelDims: 768, // Change this according to your local model's dimensions
    },
  },
  llm: {
    provider: 'ollama',
    config: {
      ...
      temperature: 0,
      maxTokens: 2000,
      ollamaBaseUrl: 'http://localhost:11434', // Ensure this URL is correct
    },
  },
  embedder: {
    provider: 'ollama',
...
// Retrieve memories
const memories = await memory.getAll({ userId: 'john' });
